...
owns the dashboard's project shape, its private exposure and its honesty
rules, and which points back here rather than restating any signal. Note
that a dashboard is *not* a signal: it has no beat of its own, so it cannot
tell you it has stopped refreshing, which is the failure mode the
live-versus-abandoned bullet above describes.
