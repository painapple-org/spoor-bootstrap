...
shopping list) is driven by the agent itself, so there's no separate manual
"now run the init skill" step.
...
4. Once you have the answers, write `.env` from `.env.example` (copy it if
   ...
   if it still has to be provisioned (step 8).
   ...
   conventions doc in step 6, not into `.env` under a name you made up.
   ...
   once they're provisioned (step 8). Leave `CONVENTIONS_DOC_PATH` blank
   here too: step 6 is what fills it in, once you've actually decided
